...
int moves[] = {140, LEFT, 90, RIGHT, 60, RIGHT, 180, RIGHT, 100, RIGHT, 60, RIGHT, 100, RIGHT, 150, LEFT};
...
void setup() {
  ...
  pinMode(pushButton, INPUT); //CHANGE TO INPUT_PULLUP
...
unsigned long Forward(int distance) {
  ...
  run_motor(A, 197); //change PWM to your calibrations
  run_motor(B, 229); //change PWM to your calibrations
  delay(t);
  run_motor(A, 0);
  run_motor(B, 0);
  return (t);
...
unsigned long Turn(int degrees) {
  ...
  run_motor(A, sign * 200); //change PWM to your calibrations
  run_motor(B, -sign * 210); //change PWM to your calibrations
  delay(t);
  run_motor(A, 0);
  run_motor(B, 0);
  return (t);
